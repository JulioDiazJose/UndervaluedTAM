# UndervaluedTAM
Script and dataset for the entitled paper "The undervalued contributions and profitability of traditional agricultural systems: a case study of the milpa in Mexico"
#-------------------------------------------------------------------------------
...
#------------------------------------------------------------------------
#2.ANALYSIS OF THE NATIONAL DATASET
#-------------------------------------------------------------------------------
# Number of units of analysis planting maize

frecuencias <- table(BASE_MAIZ_TEC_SOC$AA111_02_C)
cat("Frecuencias del producto (después del filtro): \n")
print(frecuencias)
# How many people is planting native seeds?
# We filter the number of units using native maize seeds
BASE_MAIZ_NATIVO <- filter(BASE_MAIZ_TEC_SOC,AT111_14_1 == 1)
BASE_MAIZ_NATIVO
# Frequency of 'siembra_criolla'
conteo_criolla <- table(BASE_MAIZ_NATIVO$AT111_14_1)
#Result
cat("Conteo de agricultores por tipo de siembra de semilla criolla:\n")
print(conteo_criolla)

#Individual count
si_cultivan <- conteo_criolla["1"]
no_cultivan <- conteo_criolla["2"]

cat("\nNúmero de agricultores que SÍ cultivan semilla criolla (1):", si_cultivan, "\n")
cat("Número de agricultores que NO cultivan semilla criolla (2):", no_cultivan, "\n")

# Unidades  de producción que utilizan semilla mejorada
#% de unidades de producción de utiliza semilla mejorada
#% de unidades de producción que utilizan semilla criolla

# Contar la frecuencia de cada valor
conteo_criolla <- table(BASE_MAIZ_NATIVO$AT111_14_1)

#Promedio de superficie de las unidades de producción que utilizan semilla criolla
promedio_superficie_nativa <- BASE_MAIZ_TEC_SOC %>%
  filter( AT111_14_1== 1) %>%
  summarise(promedio_superficie_nativa = mean(AA111_03_N, na.rm = TRUE))
promedio_superficie_nativa

suma_superficie_nativa <- BASE_MAIZ_TEC_SOC %>%
  filter( AT111_14_1== 1) %>%
  summarise(suma_superficie_nativa = sum(AA111_03_N, na.rm = TRUE))
suma_superficie_nativa


################################################################################
#-------------------------------------------------------------------------------
#  3. HERE WE FILTER THE MUNICIPALITIES OF INTEREST TO GET A REGIONAL
# DATABASE (27,000 OBSERVATIONS)
# -----------------------------------------------------------------------------
...
hist(base_de_datos_municipiosff19$Indice_Intensificacion_Final,
     ...
     xlab = "Índice de Intensificación (0 = Menos Intensivo, 1 = Más Intensivo)",
     ylab = "Frecuencia",
     breaks = 20, # 
     col = "darkblue",
     border = "black")
...
# the producers
#-------------------------------------------------------------------------------
# 5. WE PERFORM THE CLUSTER ANALYSIS USING THE K-MEANS ALGORITHM
# The database is provided "microdata_prac.csv" as supplementary material 
#-------------------------------------------------------------------------------
# The cluster analysis classifies producers based on agricultural practices (through 
# an index of intensification), 
# and infrastructure  such as plot area). 
...
datos_clustering_nombres1$Riego <- as.numeric(as.character(datos_clustering_nombres1$Riego))
datos_clustering_nombres1$Semilla_mejorada <- as.numeric(as.character(datos_clustering_nombres1$Semilla_mejorada))
datos_clustering_nombres1$Fertilizantes <- as.numeric(as.character(datos_clustering_nombres1$Fertilizantes))
datos_clustering_nombres1$Animales_tiro <- as.numeric(as.character(datos_clustering_nombres1$Animales_tiro))
datos_clustering_nombres1$Herbicidas <- as.numeric(as.character(datos_clustering_nombres1$Herbicidas))
datos_clustering_nombres1$Insecticidas_fungicidas <- as.numeric(as.character(datos_clustering_nombres1$Insecticidas_fungicidas))
datos_clustering_nombres1$Tractor <- as.numeric(as.character(datos_clustering_nombres1$Tractor))
datos_clustering_nombres1$Otra_maquinaria <- as.numeric(as.character(datos_clustering_nombres1$Otra_maquinaria))
datos_clustering_nombres1$Autoconsumo <- as.numeric(as.character(datos_clustering_nombres1$Autoconsumo))
datos_clustering_nombres1$Venta <- as.numeric(as.character(datos_clustering_nombres1$Venta))
datos_clustering_nombres1$Semilla_criolla <- as.numeric(as.character(datos_clustering_nombres1$Semilla_criolla))
datos_clustering_nombres1$Indigena <- as.numeric(as.character(datos_clustering_nombres1$Indigena))

datos_clustering_nombres1
str(datos_clustering_nombres1)

# Select the variables for the analysis 
datos_clusteringa <- datos_clustering_nombres1 [, c("Superficie", "Produccion", "Rendimiento", "Indice","Indigena")]

# Delete missing values NA)
datos_clusteringaa <- na.omit(datos_clusteringa)
datos_clusteringaa
# Scale numeric variables
datos_escalados <- scale(datos_clusteringaa)
datos_escalados
# Convert to a dataframe
datos_escalados <- as.data.frame(datos_escalados)

# Apply Clustering K-Means (number of clusters 3- defined by expert knowledge)
set.seed(123)
kmeans_model <- kmeans(datos_escalados, centers = 3, nstart = 25)
kmeans_model
# Add labels to the original dataset
tu_base_de_datos_clustered <- datos_clustering_nombres1 %>%
  slice(as.numeric(rownames(datos_escalados))) %>% # Asegurar el orden correcto si se eliminaron NA
  mutate(cluster = as.factor(kmeans_model$cluster))

tu_base_de_datos_clustered
##########################################


# Perform descriptive statistics for continous variables

estadisticos_cluster <- tu_base_de_datos_clustered %>%
  group_by(cluster) %>%
  summarise(
    n = n(),
...
#--Indigenous
Porcentaje_indigenas <-  tu_base_de_datos_clustered %>%
...
#--Irrigation
Porcentaje_riego <-  tu_base_de_datos_clustered %>%
...
#-Selfconsumption
Porcentaje_autoconsumo <-  tu_base_de_datos_clustered %>%
...
#-For sale
Porcentaje_venta <-  tu_base_de_datos_clustered %>%
...
#-Improved seeds
Porcentaje_semilla_mejorada <-  tu_base_de_datos_clustered %>%
...
#-Fertilizers
Porcentaje_fertilizantes <-  tu_base_de_datos_clustered %>%
...
#-Draft animals
Porcentaje_animales_tiro <-  tu_base_de_datos_clustered %>%
...
#-Herbicides
Porcentaje_herbicidas <-  tu_base_de_datos_clustered12 %>%
...
Porcentaje_insecticidas <-  tu_base_de_datos_clustered %>%
...
Porcentaje_tractor <-  tu_base_de_datos_clustered %>%
...
Porcentaje_otra_maquinaria <-  tu_base_de_datos_clustered %>%
...
ggboxplot(tu_base_de_datos_clustered12, x = "cluster", y = "Superficie", 
          color = "cluster", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("1", "2", "3"),
          ylab = "Hectáreas", xlab = "Grupo")
ggboxplot(tu_base_de_datos_clustered12, x = "cluster", y = "Produccion", 
          color = "cluster", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("1", "2", "3"),
          ylab = "Toneladas", xlab = "Grupo")

ggboxplot(tu_base_de_datos_clustered12, x = "cluster", y = "Indice", 
          color = "cluster", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("1", "2", "3"),
          ylab = "Magnitud", xlab = "Grupo")

ggboxplot(tu_base_de_datos_clustered12, x = "cluster", y = "Rendimiento", 
          color = "cluster", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("1", "2", "3"),
          ylab = "ton/ha", xlab = "Grupo")

#a) plot area
res.aovS <- aov(Superficie ~ cluster, data = tu_base_de_datos_clustered12 )
summary(res.aovS)
TukeyHSD(res.aovS)
plot(res.aovS, 2)#
#Kuskal
kruskal.test(Superficie ~ cluster, data = tu_base_de_datos_clustered12 )
pairwise.wilcox.test(tu_base_de_datos_clustered12$Superficie, tu_base_de_datos_clustered12$cluster,
                     p.adjust.method = "BH")
# b) Production
res.aovP <- aov(Produccion ~ cluster, data = tu_base_de_datos_clustered12 )
summary(res.aovP)
TukeyHSD(res.aovP)
plot(res.aovP, 2)# Kruskal
#Kuskal
kruskal.test(Produccion ~ cluster, data = tu_base_de_datos_clustered12 )
pairwise.wilcox.test(tu_base_de_datos_clustered12$Produccion, tu_base_de_datos_clustered12$cluster,
                     p.adjust.method = "BH")
# b) Yield
res.aovR <- aov(Rendimiento ~ cluster, data = tu_base_de_datos_clustered12 )
summary(res.aovR)
TukeyHSD(res.aovR)
plot(res.aovR, 2)# Kruskal
#Kuskal
kruskal.test(Rendimiento ~ cluster, data = tu_base_de_datos_clustered12 )
pairwise.wilcox.test(tu_base_de_datos_clustered12$Rendimiento, tu_base_de_datos_clustered12$cluster,
                     p.adjust.method = "BH")
head(tu_base_de_datos_clustered12)

###############################################################################
#------------------------------------------------------------------------------
# 6. PERFORM THE SIMULATION FOR THE ARQUETYPES
#------------------------------------------------------------------------------
#We assign labels by cluster instead using numbers. This is to perform
# the analysis of arquetypes.

#
tu_base_de_datos_clustered <- as.data.frame(tu_base_de_datos_clustered)
tu_base_de_datos_clustered
tu_base_de_datos_clustered$cluster <- factor(tu_base_de_datos_clustered$cluster,
                                             levels=c(1,2,3),
                                             labels = c("Tradicional","Intermedio", "Intensivo"))
print(tu_base_de_datos_clustered)
str(tu_base_de_datos_clustered$cluster)
#rename the database

base_productores_cluster <- as.data.frame(tu_base_de_datos_clustered)
...
###############################################
# 6.1.) Simulation of the economic scenario

# Here we define the distribution for the economic variables and for each unit of 
# analysis
...
resultados_simulacion <- base_productores_cluster %>%
  mutate(
    ...
    precio_venta_kg = case_when(
      cluster == "Tradicional"   ~ runif(n(), min = 9, max = 12),# Precio de venta en la localidad por ser maíz criollo
      cluster == "Intermedio"   ~ runif(n(), min = 7, max = 10),# precio de venta en la localidad puede ser maíz híbrido o criollo
      cluster == "Intensivo" ~ runif(n(), min = 6, max = 9.20) # Precio de venta a intermediarios o en la localidad directo con el consumidor o molino
    ),
    
    # Variable costs ($/ha). +/- 15% higher in the intensive system.
    costo_variable_ha = case_when(
      cluster == "Tradicional"   ~ runif(n(), min = 13991, max = 18930),
      cluster == "Intermedio"   ~ runif(n(), min = 9907, max = 13404),
      cluster == "Intensivo" ~ runif(n(), min = 17777, max = 28110)
    ),
    
    # Fixed costs ($/ha).
    costo_fijo_ha = case_when(
      cluster == "Tradicional"   ~ runif(n(), min = 419, max = 568),
      cluster == "Intermedio"   ~ runif(n(), min = 510, max = 690),
      cluster == "Intensivo" ~ runif(n(), min = 901, max = 1220)
    ),
    
    #Income/maize grain/system ($/ha)
    ingreso_maiz_ha = case_when(
      cluster == "Tradicional"   ~ runif(n(), min = 9600, max = 14400),
      cluster == "Intermedio"   ~ runif(n(), min = 6300, max = 7245),
      cluster == "Intensivo" ~ runif(n(), min = 13056, max = 25584)
    ),
    
    #Income/byproducts/system ($/ha)
    ingreso_sub_ha = case_when(
      cluster == "Tradicional"   ~ runif(n(), min = 20480, max = 30721),
      cluster == "Intermedio"   ~ runif(n(), min = 7920, max = 11880),
      cluster == "Intensivo" ~ runif(n(), min = 6060, max = 9090)
    ),
...
# Define the order
orden_personalizado <- c("Intensivo", "Intermedio", "Tradicional")# 
...
#------------------------------------------------------------------------------
# 7. ANALYSIS OF INTENSIFICATION INDEX AND NETPROFIT
# -----------------------------------------------------------------------------
...
graf4 <- ggplot(resultados_simulacion, aes(x = Indice, y = utilidad_neta)) +
  # Using GAM
  geom_smooth(method = "gam", formula = y ~ s(x), se = FALSE, color = "firebrick", linewidth = 1) +
  labs(
...
ggplot(resultados_simulacion, aes(x = Indice, y = utilidad)) +
